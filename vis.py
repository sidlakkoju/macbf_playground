...
def plot_single_state_with_wall_separate(agent_state, agent_goal, safety, border_points, wall_points, trajectories=None, wall_agent_state=None, action=None, action_res=None, action_opt=None, agent_size=100):
    if trajectories is not None:
        ...
        for i in range(num_agents):
            ...
            if trajectory.shape[0] > 0:
                plt.plot(trajectory[:, 0], trajectory[:, 1], linestyle='--', color=cmap(i % 10), label=f'Agent {i} Trajectory')
    ...
    plt.scatter(border_points[:, 0], border_points[:, 1], label='Border Points', color='blue', s=10)
    plt.scatter(wall_points[:, 0], wall_points[:, 1], label='Wall Points', color='green', s=10)
    ...
    if action is not None:
        plt.quiver(agent_state_vis[:, 0], agent_state_vis[:, 1], action[:, 0], action[:, 1],
                   color='green', scale=1, label='Action')
        for i in range(agent_state_vis.shape[0]):
            plt.text(agent_state_vis[i, 0] + 0.05, agent_state_vis[i, 1] + 0.05,
                     f'{action[i]}', color='green', fontsize=8)

    if action_res is not None:
        plt.quiver(agent_state_vis[:, 0], agent_state_vis[:, 1], action_res[:, 0], action_res[:, 1],
                   color='blue', scale=1, label='Action Res')
        for i in range(agent_state_vis.shape[0]):
            plt.text(agent_state_vis[i, 0] + 0.05, agent_state_vis[i, 1] - 0.05,
                     f'{action_res[i]}', color='blue', fontsize=8)

    if action_opt is not None:
        plt.quiver(agent_state_vis[:, 0], agent_state_vis[:, 1], action_opt[:, 0], action_opt[:, 1],
                   color='purple', scale=1, label='Action Opt')
        for i in range(agent_state_vis.shape[0]):
            plt.text(agent_state_vis[i, 0] + 0.05, agent_state_vis[i, 1] - 0.1,
                     f'{action_opt[i]}', color='purple', fontsize=8)
