...
class ActionNetwork(nn.Module):
    ...
    def forward(self, s, g, neighbor_features):
        x = neighbor_features.permute(0, 2, 1)
        x = torch.relu(self.conv1(x))
        x = torch.relu(self.conv2(x))
...
def compute_neighbor_features(s, r, k):
    ...
    neighbor_features = torch.cat([neighbor_features, eye, d_norm], dim=2)
...
# Loss functions
def barrier_loss(h, s, r, ttc):
    mask = ttc_dangerous_mask(s, r, ttc)
    h = h.view(-1)
    mask = mask.view(-1)
    dang_h = h[mask]
    safe_h = h[~mask]
    loss_dang = torch.mean(torch.relu(dang_h + 1e-3))
    loss_safe = torch.mean(torch.relu(-safe_h))
    return loss_dang + loss_safe

def derivative_loss(h, s, a, cbf_net, alpha):
    s_next = s + dynamics(s, a) * config.TIME_STEP
    neighbor_features = compute_neighbor_features(s_next, config.DIST_MIN_THRES, config.TOP_K)
...
def train(num_agents):
    ...
    for step in range(config.TRAIN_STEPS):
        ...
        for _ in range(config.INNER_LOOPS):
            neighbor_features = compute_neighbor_features(s, config.DIST_MIN_THRES, config.TOP_K)
            h = cbf_net(neighbor_features)
            a = action_net(s, g, neighbor_features)
            loss_b = barrier_loss(h, s, config.DIST_MIN_THRES, config.TIME_TO_COLLISION)
            loss_d = derivative_loss(h, s, a, cbf_net, config.ALPHA_CBF)
