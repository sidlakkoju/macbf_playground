    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        for i in range(config.INNER_LOOPS):
            ...
            for _ in range(config.REFINE_LOOPS):
                ...
                neighbor_features_cbf_next, _ = core.compute_neighbor_features(s_next, config.DIST_MIN_THRES, config.TOP_K, include_d_norm=True, indices=indices)
                h_next = cbf_net(neighbor_features_cbf_next)
                deriv = h_next - h + config.TIME_STEP * config.ALPHA_CBF * h        # Question: What is the point of the adding term (config.TIME_STEP * config.ALPHA_CBF * h)?
            ...
            neighbor_features_cbf_next, _ = core.compute_neighbor_features(s_next, config.DIST_MIN_THRES, config.TOP_K, include_d_norm=True, indices=indices)
            h_next = cbf_net(neighbor_features_cbf_next)
            loss_dang, loss_safe, acc_dang, acc_safe = core.barrier_loss(h, s, config.DIST_MIN_THRES, config.TIME_TO_COLLISION, indices)
            loss_dang_deriv, loss_safe_deriv, acc_dang_deriv, acc_safe_deriv = core.derivative_loss(h, s, a, cbf_net, config.ALPHA_CBF, indices)
        ...
        if args.vis:
            ...
            for j in range(max_steps):
                ...
                core.plot_single_state_with_original(state_ours, g_np, safety_ours_current, s_np_ori, agent_size)
                ...
                core.plot_single_state_with_original(state_lqr, g_np, safety_lqr_current, s_np_ori, agent_size)
